...
function FormSubmitComponent({formUrl, content}: {formUrl: string; content: FormElementInstance[]}) {
  ...
  const validateFrom = () => {
    formErrors.current = {}
    // button submitForm 클릭시, value값이 유효한지 한번에 검사
    ...
    return (
      <div className="flex justify-center items-center w-full h-full p-8">
        <div className="max-w-[620px] flex flex-col gap-4 flex-grow bg-background w-full p-8 overflow-y-auto border shadow-xl shadow-blue-700 rounded">
          <h1 className="text-2xl font-bold border-b-2 py-3">Form submitted</h1>
  ...
  return (
    <div className="flex justify-center w-full h-full items-start p-8">
      <div
        key={renderKey}
